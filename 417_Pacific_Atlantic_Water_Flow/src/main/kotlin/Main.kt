...
class Solution2 {
    ...
    fun pacificAtlantic(heights: Array<IntArray>): List<List<Int>> {
        ...
        n = heights[0].size
        val result = mutableListOf<List<Int>>()
        val reach = Array(m){Array(n){BooleanArray(2)}}.apply {

        }

        for(i in 0 until m){
            for(j in 0 until n){
                dfsHelper(heights, i, j, reach, Array(m){ BooleanArray(n) }, i, j)
//                reach[i][j][PACIFIC] = r[PACIFIC]
//                reach[i][j][ATLANTIC] = r[ATLANTIC]
    ...
    private fun dfsHelper(
        ...
        for(dir in dirs){
            ...
            val y = j + dir[1]
            if((x in 0 until m) && (y in 0 until n) && heights[i][j]>=heights[x][y]){
                if(!visited[x][y] && !(reach[x][y][PACIFIC] && reach[x][y][ATLANTIC])){
                    dfsHelper(heights, x, y, reach, visited, oriI, oriJ)
                }
        ...
        reach[i][j][PACIFIC] = pacific
        reach[i][j][ATLANTIC] = atlantic
    }
}